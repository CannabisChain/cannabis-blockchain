...
from cannabis.protocols import full_node_protocol, introducer_protocol
from cannabis.protocols.protocol_message_types import ProtocolMessageTypes
from cannabis.server.address_manager import AddressManager, ExtendedPeerInfo
from cannabis.server.address_manager_store import AddressManagerStore
from cannabis.server.outbound_message import NodeType, make_msg
from cannabis.server.server import CannabisServer
from cannabis.types.peer_info import PeerInfo, TimestampedPeerInfo
...
class FullNodeDiscovery:
    ...
    def __init__(
        ...
        self.server: CannabisServer = server
    ...
    async def on_connect(self, peer: ws.WSCannabisConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSCannabisConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSCannabisConnection):

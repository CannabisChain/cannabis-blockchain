...
from cannabis.cmds.init_funcs import cannabis_init
from cannabis.daemon.windows_signal import kill
from cannabis.server.server import ssl_context_for_root, ssl_context_for_server
from cannabis.ssl.create_ssl import get_mozilla_ca_crt
from cannabis.util.cannabis_logging import initialize_logging
...
from cannabis.util.service_groups import validate_service
from cannabis.util.setproctitle import setproctitle
from cannabis.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Cannabis.")
    quit()
...
service_plotter = "cannabis plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "cannabis": "cannabis",
        "cannabis_wallet": "start_wallet",
        "cannabis_full_node": "start_full_node",
        "cannabis_harvester": "start_harvester",
        "cannabis_farmer": "start_farmer",
        "cannabis_introducer": "start_introducer",
        "cannabis_timelord": "start_timelord",
        "cannabis_timelord_launcher": "timelord_launcher",
        "cannabis_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("cannabis daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered CANNABIS_ROOT
    os.environ["CANNABIS_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with CANNABIS_ROOT: {os.environ['CANNABIS_ROOT']}")
...
async def async_run_daemon(root_path: Path) -> int:
    cannabis_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("cannabis_daemon")

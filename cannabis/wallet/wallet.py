...
from cannabis.consensus.cost_calculator import calculate_cost_of_program, NPCResult
from cannabis.full_node.bundle_tools import simple_solution_generator
...
from cannabis.types.blockchain_format.program import Program, SerializedProgram
from cannabis.types.announcement import Announcement
...
from cannabis.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
    solution_for_conditions,
)
from cannabis.wallet.puzzles.puzzle_utils import (
...
from cannabis.wallet.secret_key_store import SecretKeyStore
from cannabis.wallet.sign_coin_solutions import sign_coin_solutions
...
class Wallet:
    ...
    async def create_spend_bundle_relative_cannabis(self, cannabis_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if cannabis_amount < 0:
            utxos = await self.select_coins(abs(cannabis_amount), exclude)
        else:
        ...
        cannabis_amount = spend_value + cannabis_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": cannabis_amount}]
